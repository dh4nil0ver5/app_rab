...
<script> 
  $("#saveUpMaster").click(function() {
    $.ajax({
      type: "post",
      url: "<?php echo url('update/master'); ?>",
      data: {
        id_work: $("#up_id_pekerjaan").val(),
        name_ofpointwork: $("#up_master_pekerjaan").val()
      },
      dataType: "json",
  ...
  function check(param){   
    $.post({
      url: "<?php echo url('/'); ?>/idmaster", 
      data: {
        id_master: $("#sub"+param+"").attr("kode")
      },
      dataType: 'json',
      success: function (json) { 
        if (parseInt(json['status']) == 200) { 
          $.each(json['data'], function (item, object) {  
              $("#up_id_pekerjaan").val(object.id_pointofwork); 
              $("#up_master_pekerjaan").val(object.name_ofpointwork);  
          });
          $("#updateMaster").modal('toggle'); 
        } else {
          $("#updateMaster").modal('toggle');
  ...
  function hapus(param){ 
    $.get({
      url: "<?php echo url('/'); ?>/hapus/master/"+$("#sub"+param).attr('kode'),  
      dataType: 'json',
      success: function (json) { 
        if (parseInt(json['status']) == 200) {   
          Swal.fire('sukses !');
