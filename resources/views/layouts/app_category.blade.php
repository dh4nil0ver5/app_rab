...
<script> 
  $("#updateFormula").click(function() {
    $.ajax({
      type: "post",
      url: "<?php echo url('update/category/pekerjaan'); ?>",
      data: { 
        id_category: $("#up_idkategori").val(),
        name_ofcategory: $("#up_namekategori").val()
      },
      dataType: "json",
      success: function (json) {
        if (parseInt(json['status']) == 200) { 
          $("#upCategory").modal('toggle');
          Swal.fire('sukses !');
          loaddata();
        } else {
          $("#upCategory").modal('toggle');
  ...
  $("#saveFormula").click(function() {
    $.ajax({
      type: "post",
      url: "<?php echo url('save/category/pekerjaan'); ?>",
      data: { 
        name_ofcategory: $("#id_namekategori").val()
      },
      dataType: "json",
      success: function (json) {
        if (parseInt(json['status']) == 200) { 
  ...
  function check(param){   
    ...
    $.get({
      url: "<?php echo url('/'); ?>/data/category/pekerjaan/"+$("#sub"+param).attr('kode'), 
      dataType: 'json',
      success: function (json) { 
        if (parseInt(json['status']) == 200) {  
          $.each(json['data'], function (i, item) {   
            $("#up_idkategori").val(item.id_category);
            $("#up_namekategori").val(item.name_ofcategory);
          });
          $("#upCategory").modal('toggle'); 
        } else {
          $("#upCategory").modal('toggle'); 
  ...
  function hapus(param){ 
    $.post({
      url: "<?php echo url('/'); ?>/delete/category/pekerjaan",  
      data: { 
        id_category: $("#sub"+param).attr('kode'), 
      },
      dataType: 'json',
      success: function (json) { 
        if (parseInt(json['status']) == 200) {   
  ...
  function loaddata(){ 
    ...
    $("#list_formula").DataTable({
        ...
        bInfo: false,
        ajax: {
          url: "<?php echo url('data/category/pekerjaan'); ?>",
          type: "get",
          dataSrc: function (json) {
            var return_data = new Array();
              ...
              $.each(json['data'], function (i, item) {  
	                return_data.push({
                    'no':(i+1),
                    'name_ofitem':item.name_ofcategory, 
                    'aksi':
                    "<div class='btn btn-xs btn-warning btn-flat' kode='"+item.id_category+"' id='sub"+i+"' onclick='check("+i+")'>ubah</div>"+
