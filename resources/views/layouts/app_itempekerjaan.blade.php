...
<script> 
  $("#saveUpMaster").click(function() {
    $.ajax({
      type: "post",
      url: "<?php echo url('update/item/pekerjaan'); ?>",
      data: {
        id_item: $("#up_id_item").val(),
        name_ofitem: $("#up_name_ofitem").val()
      },
      dataType: "json",
  ...
  $("#saveMaster").click(function() {
    $.ajax({
      type: "post",
      url: "<?php echo url('save/item/pekerjaan'); ?>",
      data: { 
        name_ofitem: $("#name_ofitem").val()
      },
      dataType: "json",
      success: function (json) {
        if (parseInt(json['status']) == 200) { 
  ...
  function check(param){   
    $.get({
      url: "<?php echo url('/'); ?>/data/item/pekerjaan/"+$("#sub"+param).attr('kode'), 
      dataType: 'json',
      success: function (json) { 
        if (parseInt(json['status']) == 200) { 
          $.each(json['data'], function (item, object) {   
              $("#up_id_item").val(object.id_item);  
              $("#up_name_ofitem").val(object.name_ofitem);  
          });
          $("#updateMaster").modal('toggle'); 
        } else {
          $("#updateMaster").modal('toggle');
  ...
  function hapus(param){ 
    $.post({
      url: "<?php echo url('/'); ?>/delete/item/pekerjaan",  
      data: { 
        id_item: $("#sub"+param).attr('kode'), 
      },
      dataType: 'json',
      success: function (json) { 
        if (parseInt(json['status']) == 200) {   
  ...
  function loaddata(){ 
    ...
    $("#lista_proiecte").DataTable({
        ...
        bInfo: false,
        ajax: {
          url: "<?php echo url('data/item/pekerjaan'); ?>",
          type: "get",
          dataSrc: function (json) {
            var return_data = new Array();
              ...
              $.each(json['data'], function (i, item) { 
	                return_data.push({
                    'no':(i+1),
                    'nama_work':item.name_ofitem, 
                    'aksi':"<div class='btn btn-xs btn-warning btn-flat' kode='"+item.id_item +"' id='sub"+i+"' onclick='check("+i+")'>edit</div>"+
