...
<script>
  $("#saveUpMaster").click(function() {
    $.ajax({
      type: "post",
      url: "<?php echo url('update/pekerjaan'); ?>",
      data: {
  ...
  $("#saveMaster").click(function() {
    $.ajax({
      type: "post",
      url: "<?php echo url('save/pekerjaan'); ?>",
      data: {
  ...
  function check(param){  
    ...
    $.get({
      url: "<?php echo url('/'); ?>/ambil/pekerjaan/"+$("#sub"+param).attr('kode'),  
      dataType: 'json',
      success: function (json) { 
        if (parseInt(json['status']) == 200) { 
  ...
  function hapus(param){ 
    $.ajax({
      type: "post",
      url: "<?php echo url('delete/pekerjaan'); ?>",
      data: {
        id_work: $("#sub"+param).attr('kode'), 
      },
      dataType: "json",
      success: function (json) {
        if (parseInt(json['status']) == 200) {  
  ...
  function loaddata(){ 
    $("#list_pekerjaan").DataTable().destroy();
    $("#list_pekerjaan").DataTable({
        ...
        bInfo: false,
        ajax: {
          url: "<?php echo url('data/pekerjaan'); ?>",
          type: "get",
          dataSrc: function (json) {
            var return_data = new Array(); 
              ...
              $.each(json['data'], function (i, item) { 
                    ...
                    'aksi':"<div class='btn btn-xs btn-warning btn-flat' kode='"+item.id_work+"' id='sub"+i+"' onclick='check("+i+")'>edit</div>"+
